    ...
    <>
      ...
      <section style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e3a5f 60%, #1a56db 100%)', padding: '56px 24px', textAlign: 'center' }}>
        <div style={{ maxWidth: 600, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 12, marginBottom: 8 }}>
            ...
            <span style={{ fontSize: 36, fontWeight: 900, color: '#f59e0b' }}>{avgRating}</span>
      ...
      <div className="container" style={{ padding: '48px 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 48 }}>
          {[
            { num: avgRating, label: 'Durchschnittsbewertung', stars: true },
            { num: `${totalReviews}`, label: 'Bewertungen gesamt', stars: false },
          ...
          {reviews.map(review => (
            <div key={review.id} style={{ background: '#fff', borderRadius: 16, border: '1px solid #e2e8f0', overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
              ...
              {review.has_photo && review.product_img && (
                <div
                  onClick={() => setLightbox(review.product_img)}
                  style={{ margin: '0 24px 16px', borderRadius: 10, overflow: 'hidden', background: '#f8fafc', border: '1px solid #e2e8f0', cursor: 'zoom-in', aspectRatio: '16/9', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                >
              ...
              {/* Content */}
              <div style={{ padding: review.has_photo ? '0 24px 20px' : '0 24px 20px' }}>
      ...
      {lightbox && (
        ...
        >
          ...
          <button onClick={() => setLightbox(null)} style={{ position: 'absolute', top: 20, right: 20, background: 'rgba(255,255,255,0.15)', border: 'none', color: '#fff', fontSize: 24, width: 44, height: 44, borderRadius: '50%', cursor: 'pointer' }}>✕</button>
        </div>
      )}
