...
export default function Home() {
  ...
  useEffect(() => {
    const promises = Array.from({ length: TOTAL_CHUNKS }, (_, i) =>
  ...
  return (
    <>
      ...
      <section style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e3a5f 50%, #1a56db 100%)', padding: '64px 24px', textAlign: 'center' }}>
        <div style={{ maxWidth: 640, margin: '0 auto' }}>
          <div style={{ display: 'inline-block', background: 'rgba(59,130,246,0.2)', border: '1px solid rgba(59,130,246,0.5)', color: '#93c5fd', padding: '5px 16px', borderRadius: 20, fontSize: 12, fontWeight: 700, letterSpacing: '1.2px', textTransform: 'uppercase', marginBottom: 20 }}>
            ⚡ Bis zu 30% Rabatt
          </div>
          <h1 style={{ fontSize: 'clamp(36px, 5vw, 56px)', fontWeight: 900, color: '#f8fafc', lineHeight: 1.1, marginBottom: 16, letterSpacing: '-1px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 48 }}>
            {[
              { num: loading ? '...' : `${filtered.length}`, label: 'Produkte' },
      ...
      <main className="container" style={{ padding: '40px 24px' }}>
        <div style={{ display: 'flex', gap: 36, alignItems: 'flex-start' }}>
          <div style={{ position: 'sticky', top: 80, flexShrink: 0 }}>
            <Filters brands={ALL_BRANDS} aktivitaeten={ALL_AKTIVITAETEN} filters={filters} onChange={f => { setFilters(f); setPage(1) }} />
          </div>
