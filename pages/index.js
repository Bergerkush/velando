...
export default function Home({ initialProducts }) {
  ...
  const ALL_BRANDS = useMemo(() => {
    return [...new Set(initialProducts.map(p => p.brand).filter(Boolean))].sort()
  }, [initialProducts])
  ...
  const filtered = useMemo(() => {
    ...
    return list
  }, [search, filters, sort, initialProducts])
...
export async function getStaticProps() {
  ...
  const possiblePaths = [
    ...
    path.join(process.cwd(), '..', 'public', 'products.json'),
    path.join(process.cwd(), 'velando-next', 'public', 'products.json'),
  ]
