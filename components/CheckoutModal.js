...
export default function CheckoutModal({ open, onClose }) {
  ...
  return (
    <div
      style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.5)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, backdropFilter: 'blur(4px)' }}
        ...
        {success ? (
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ background: '#f8fafc', borderRadius: 10, padding: 16, marginBottom: 24, border: '1px solid #e2e8f0' }}>
                ...
                <div key={`${i.id}-${i.selectedSize}-${idx}`} style={{ marginBottom: 8, paddingBottom: 8, borderBottom: idx < items.length - 1 ? '1px solid #e2e8f0' : 'none' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: '#334155' }}>
                    <span style={{ flex: 1, marginRight: 8 }}>
                      {i.name.slice(0, 35)}{i.name.length > 35 ? '…' : ''} ×{i.qty}
                    </span>
                  ...
                  {i.selectedSize && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 4 }}>
                      ...
                      <span style={{ display: 'inline-block', background: '#eff6ff', color: '#1a56db', padding: '1px 8px', borderRadius: 5, fontSize: 11, fontWeight: 700 }}>
                        {i.selectedSize}
                      </span>
                      {i.sizes && i.sizes[i.selectedSize] === 'on_request' && (
                        <span style={{ fontSize: 11, color: '#f59e0b', fontWeight: 600 }}>⏱ Auf Anfrage</span>
                      )}
                    </div>
                  )}
                </div>
              ...
              {/* Предупреждение если есть товары под заказ */}
            ...
            <button type="submit" style={primaryBtnStyle}>Bestellung absenden ✓</button>
          </form>
        )}
      </div>
    </div>
...
const primaryBtnStyle = {
  width: '100%', padding: 15, background: '#1a56db', color: '#fff',
