  ...
  return (
    <>
      ...
      <style>{`
        ...
        .filters-mobile-fab { display: none; }
        @media (max-width: 768px) {
          .filters-desktop { display: none !important; }
          .filters-mobile-fab { display: flex !important; }
        }
      `}</style>

      <aside className="filters-desktop" style={{ width: 260, flexShrink: 0 }}>
        {filtersContent}
      </aside>

      {/* Mobile FAB button — always in DOM, shown via CSS */}
      <button
        className="filters-mobile-fab"
        onClick={() => setMobileOpen(true)}
        style={{
          position: 'fixed', bottom: 20, left: '50%', transform: 'translateX(-50%)',
          zIndex: 150, display: 'none', alignItems: 'center', gap: 8,
